...
import { AlertSeverity, AlertStatus, ServiceStatus } from '@shared/types';
...
export function useUrlState() {
  ...
  useEffect(() => {
    ...
    const alertServiceId = params.get('alertServiceId') || 'all';

    if (alertStatus || alertSeverity || alertSearch || alertServiceId !== 'all') {
      setAlertFilters({
        status: alertStatus || 'all',
        severity: alertSeverity || 'all',
        search: alertSearch,
        serviceId: alertServiceId
      });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (alertFilters.serviceId !== 'all') params.set('alertServiceId', alertFilters.serviceId);
